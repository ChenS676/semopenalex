...
today = date.today()
print(f"Overall start -- {start_time}.")
...
nt_output_dir_path = f'/mnt/lsdf_sqa/public/nt_v2/{ENTITY_TYPE}/'
...
def transform_gz_file(gz_file_path):
    ...
    with open(f"{nt_output_dir_path}{gz_file_name}.nt", "w", encoding="utf-8") as g:
        with gzip.open(gz_file_path, 'r') as f:
            ...
            for line in f:
                ...
                if i % 50000 == 0:
                    print(f'Processed {i} lines in file {gz_file_path}')

                if i % 50000 == 0:
                    g.write(works_graph.serialize(format='nt'))
                    works_graph = Graph()
        ...
        f"{nt_output_dir_path}{gz_file_name}.nt"

    f.close()
    g.close()

    print(f"Worker completed .nt transformation with {i} lines and {file_error_count} errors")
    ...
    os.system(f'gzip --fast -k {nt_output_dir_path}{gz_file_name}.nt')
